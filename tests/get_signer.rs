...
fn sample_user_market_order() -> UserMarketOrder {
...
async fn order_builder_get_signer_uses_factory_signer_for_build_and_sign() {
    ...
    assert_eq!(signed.order.maker, dynamic_signer.address());
    assert_eq!(
        signed
            .signature
            .recover_address_from_prehash(&hash)
...
#[tokio::test]
async fn market_order_builder_get_signer_uses_factory_signer_for_build_and_sign() {
    let client = configured_client(Config::default()).await;
    let static_signer = common::signer();
    let dynamic_signer = alternate_signer();
...
async fn get_signer_factory_error_propagates() {
    let client = configured_client(
        Config::default().get_signer(|| async { Err(Error::validation("dynamic signer failure")) }),
    )
    .await;
...
#[tokio::test]
async fn config_get_signer_market_order_uses_dynamic_signer_as_maker_and_signer() {
    let static_signer = common::signer();
    let dynamic_signer = alternate_signer();
    let client = configured_client(Config::default().get_signer({
        let dynamic_signer = dynamic_signer.clone();
    ...
    let signed = client
        .create_market_order(&static_signer, sample_user_market_order())
        .await
